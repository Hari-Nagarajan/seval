...
Open or update a GitHub Pull Request for seval. Handles creating new PRs with a fully filled-out template body, and updating existing PRs (title, body sections, labels, comments). Use this skill whenever the user wants to open a PR, create a pull request, update a PR, edit PR description, add labels to a PR, or sync a PR after new commits — even if they don't say "PR" explicitly (e.g., "submit this for review", "push and open for merge").
...
cargo clippy --all-targets -- -D clippy::correctness 2>&1 | tail -3
...
- Draft a conventional commit-style PR title based on the changes (e.g., `feat(agents): add spawn tool`, `fix(chat): handle system messages`, `chore(ci): update workflow permissions`).
...
- **Always read `.github/pull_request_template.md`** before filling or editing a PR body. Never assume section names, fields, or structure — derive everything from the template.
